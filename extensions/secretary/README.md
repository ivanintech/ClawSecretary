...
### 1. El "Big Bang" (Instalación)
Ejecuta el asistente en tu PC/Mac:
```bash
npx openclaw-secretary@latest
```
Verás una interfaz premium que prepara todo en segundos.

### 2. El Enlace Mágico (Emparejamiento QR)
Escanea el código QR que aparecerá en tu terminal. Tu móvil abrirá instantáneamente el **Dashboard PWA** configurado y listo para funcionar.

### 3. Social Sync (OAuth 2.0)
Conecta Google, Notion o WhatsApp con un click. Los tokens se inyectan directamente en tu dispositivo local mediante nuestro puente seguro y efímero.
