...
export function createWhatsAppTool(_api: OpenClawPluginApi) {
    return {
        ...
        parameters: Type.Object({
            action: Type.String({
                enum: ["send_text", "send_buttons", "send_list"],
                description: "Type of WA message to send.",
            }),
            to: Type.String({ description: "Recipient phone number in international format without + (e.g. 34612345678)." }),
            ...
            listButtonLabel: Type.Optional(Type.String({ description: "The CTA button label that opens the list (default: 'Ver opciones')." })),
        ...
        async execute(_runId: string, params: Record<string, any>) {
            ...
            if (!apiKey) {
                return {
                    content: [{ type: "text" as const, text: "⚠️ `MATON_API_KEY` no configurada. WhatsApp Business no disponible." }],
                };
            }
            if (!phoneNumberId) {
                return {
            ...
            if (params.action === "send_buttons") {
                ...
                return {
                    content: [{ type: "text" as const, text: `✅ Mensaje con botones enviado a ${params.to}\nBotones: ${buttons.join(" | ")}` }],
                    details: { result },
                };
            }
